...
class DashboardController extends BaseController
{
    ...
    {
        $data['pageTitle'] = 'Home';
        // get current cycle of two cycles based on current date
        // first cycle is from january to june
        // second cycle is from july to december
        $current_month = date('m');
        $current_year = date('Y');
        $current_cycle = ($current_month >= 1 && $current_month <= 6) ? '1' : '2';
        ...
        $loan->select('SUM(amount_topay - balance) as collection'); // get collection = amount_topay minus balance (total of amortization paid)
        $loan->select('SUM(balance) as current_collectible'); // get current_collectible = total of amortization NOT YET paid
        $loan->where('YEAR(loan_date)', $current_year);
        $loan->where('MONTH(loan_date) >=', ($current_cycle == '1') ? 1 : 7);
        $loan->where('MONTH(loan_date) <=', ($current_cycle == '1') ? 6 : 12);
        ...
        return view('information/dashboard', $data);
    }
}